 ...
 * @license MIT
 *
 * Modified by __root__ on 07-October-2025 using Strauss.
...
namespace CUMULUS\Gutenberg\Tools\Vendors\vena\WordpressSettingsBuilder;

\define( 'CUMULUS\\Gutenberg\\Tools\\Vendors\\vena\\WordpressSettingsBuilder\\BUILDDIR', __DIR__ . '/../../build' );
...
class Builder {
	...
	public function do_settings_fields( $page, $section ) {
		...
		foreach ( (array) $wp_settings_fields[ $page ][ $section ] as $field ) {
			...
			if ( \in_array( 'required', $field['args'], true ) ) {
				$classes[] = 'required';
			}
			...
			if ( isset( $field['title'] ) && $field['title'] ) {
				if ( ! empty( $field['args']['label_for'] ) ) {
					\printf(
						'<label for="%s" %s class="title">%s</label>',
						\esc_attr( $field['args']['label_for'] ),
						\in_array( 'required', $field['args'], true ) ? ' title="This field is required"' : '',
						\is_bool( $field['title'] ) ? '' : \wp_kses_post( $field['title'] )
					);
				} else {
					\printf(
