...
class Radio {
	...
	public function outputField( $args ) {
		...
		foreach ( $args['options'] as $option ) {
			?>
				<?php echo ! isset( $args['inline'] ) || $args['inline'] === false ? '<p>' : ''; ?>
				<label for="<?php echo \esc_attr( $args['label_for'] ); ?>[<?php echo $index; ?>]">
					<input
						type="<?php echo \esc_attr( $this->type ); ?>"
						id="<?php echo \esc_attr( $args['label_for'] ); ?>[<?php echo $index; ?>]"
