...
class Select {
	...
	public function outputField( $args ) {
		...
		>
			<option><?php echo empty( $args['no_value'] ) ? '' : \esc_html( $args['no_value'] ); ?></option>
			<?php foreach ( $args['options'] as $option ): ?>
				<?php $this->outputOption( $option, $value ); ?>
			<?php endforeach; ?>
		</select>
	...
	private function outputOption( $option, $value ) {
		if ( empty( $option['options'] ) ) {
			...
			?>
				<optgroup label="<?php echo empty( $option['title'] ) ? '' : \esc_attr( $option['title'] ); ?>">
					<?php foreach ( $option['options'] as $optGroupOptions ): ?>
						<?php $this->outputOption( $optGroupOptions, $value ); ?>
					<?php endforeach; ?>
				</optgroup>
			<?php
		}
